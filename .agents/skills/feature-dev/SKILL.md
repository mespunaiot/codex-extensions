...
IMPORTANT: If temporary investigation files are found in tracked paths, delete them (or keep them completely unstaged) before committing and pushing.
...
8. End the PR body with this exact line:
   - `Generated by {{RUNTIME_MODEL_ID}}`

## GitHub Footer Model Resolution (Mandatory)
- Resolve `{{RUNTIME_MODEL_ID}}` at runtime before posting any GitHub-facing text.
- Use the first non-empty environment variable in this order:
  - `OPENAI_MODEL`
  - `CODEX_MODEL`
  - `MODEL`
  - `LLM_MODEL`
- Use the value as-is. Prefer the most specific runtime value provided by environment.
- Do not invent or guess a more specific model variant than the runtime provides.
- If none of the variables are available, use this exact fallback footer line:
  - `Generated by Codex (runtime model id unavailable)`
- Recommended helper (use this output instead of manual typing):
